...
if [ "$MARSCODE_DEV_MODE" ]; then
    ...
    exec ./binary/ckg_server -port=${PORT0} -ide_version=${ICUBE_BUILD_VERSION} -version_code=2 -storage_path="$ICUBE_MODULAR_DATA_DIR/ckg_server" -local_embedding -embedding_storage_type=sqlite_vec -app_id=$CKG_APP_ID -limit_cpu=1 -source_product=$CKG_SOURCE_PRODUCT
fi
